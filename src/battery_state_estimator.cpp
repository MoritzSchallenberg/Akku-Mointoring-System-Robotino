...
public:
  ...
  : Node("battery_state_estimator")
  {
    ...
    // Der alte Fawkes-Code hatte ungefähr bei 17.2 V eine Warnschwelle.
    u_min_v_ = this->declare_parameter<double>("u_min_v", 17.2);

    // u_max_v:
    // Spannung eines vollen 18-V-Akkusystems unter realen Bedingungen.
    // Ein 5s-Li-Ion-Pack kann voll theoretisch 21.0 V haben, praktisch sind
    // 20.5 V als Startwert oft sinnvoller.
    u_max_v_ = this->declare_parameter<double>("u_max_v", 20.5);

    // u_error_v:
    // Sicherheitsabschlag auf die Spannung, damit die Spannungsschätzung
    ...
    // runtime_min_current_a:
    // Unterhalb dieses Durchschnittsstroms berechnen wir keine sinnvolle Restlaufzeit,
    // weil die Rechnung bei sehr kleinen Strömen unrealistisch große Werte ergibt.
    ...
    // temp_ref_c und k_u_t_v_per_c:
    // Optionaler Temperaturausgleich für die Spannung.
    // Aktuell bleibt k_u_t_v_per_c standardmäßig 0.0, weil eure Temperaturumrechnung
    // noch nur eine grobe NTC-Schätzung ist.
    temp_ref_c_ = this->declare_parameter<double>("temp_ref_c", 25.0);
    k_u_t_v_per_c_ = this->declare_parameter<double>("k_u_t_v_per_c", 0.0);

    // Wenn true, wartet die Node beim Start auf eine gültige Stillstandsspannung
    // und initialisiert daraus die Restladung. Wenn keine gültige Spannung kommt,
    // startet sie erstmal mit voller Kapazität.
...
private:
  ...
  void parseArduinoState(const std::string & line)
  {
    ...
    // Dadurch muss diese Node keinen eigenen Moving Average mehr berechnen.
    getDouble(values, "current_avg_a", latest_average_current_a_);

    // Temperatur kommt jetzt bereits als grobe °C-Schätzung vom Arduino.
  ...
  void parseRobotinoVoltageState(const std::string & line)
  {
    ...
    // Spannung in Prozent bezogen auf Vollspannung. Wird nicht direkt für die Rechnung genutzt,
    // aber im State wieder ausgegeben, weil es für Anzeige/Debug praktisch ist.
  ...
  double calculateVoltageBasedCapacityAh(double voltage_v, double temperature_c) const
  {
    ...
    // Wenn k_u_t_v_per_c = 0.0, ist diese Zeile wirkungslos.
  ...
  void initializeRemainingAhIfNeeded()
  {
    ...
    // Beste Variante: Wir haben bereits eine gültige Stillstandsspannung.
    // Dann starten wir die Restladung nicht blind bei 100 %, sondern anhand der Spannung.
    if (initialize_from_voltage_when_available_ && has_voltage_ && voltage_idle_valid_) {
      ...
      // Fallback: Wenn noch keine gültige Spannung da ist, starten wir konservativ
      // mit der aktuell gemeldeten Kapazität.
  ...
  void updateCalculation()
  {
    ...
    // Wir nehmen deshalb genau diesen Wert und nicht mehr einen eigenen Durchschnitt.
    const double average_current_a = std::abs(latest_average_current_a_);

    // Ah-Verbrauch des letzten Berechnungsintervalls.
    // Beispiel: 4 A für 10 Sekunden -> 4 * 10 / 3600 Ah.
    ...
    // dass keine gültige Spannungskorrektur angewendet wurde.
    ...
    if (has_voltage_ && voltage_idle_valid_) {
      ...
      // Wenn keine gültige Stillstandsspannung vorhanden ist,
      // läuft die Rechnung rein über Coulomb Counting weiter.
    ...
    // Finalen State bauen
    // ------------------------------------------------------------
    // Dieser eine State ersetzt die vielen Einzel-Topics.
    // Andere Nodes können später nur diesen einen Topic abonnieren.
