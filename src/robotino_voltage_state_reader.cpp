...
public:
  ...
  : Node("robotino_voltage_state_reader")
  {
    ...
    //   Wenn der Betrag des Stroms kleiner als dieser Wert ist, betrachten wir den Robotino als "nahe Stillstand".
    this->declare_parameter<double>("idle_current_threshold_a", 0.5);

    // idle_required_s:
    //   Der Strom muss so lange unterhalb des Schwellwerts bleiben, bevor die Spannung als gültige Ruhespannung veröffentlicht wird.
...
private:
  ...
  std::string http_get(const std::string &url)
  {
    ...
    // Kurze Timeouts, damit die Node nicht zu lange blockiert, falls der Robotino gerade nicht erreichbar ist.
