...
RobotinoSensorThread::loop()
{
	...
	// Erweiterte Akku-Auswertung: ACS758, Slots, Temperatur, Coulomb Counting
...
{
	// data.bat_voltage wird im vorhandenen Code als mV behandelt.
...
{
	// Annahme: data.analog_in[] enthält bereits Voltwerte.
	// ACS758-200B: I = (U_out - U_zero) / Sensitivität.
...
RobotinoSensorThread::update_coulomb_counting(float dt_s)
{
	...
	// Entnommene Kapazität: Ah = A * s / 3600
	if(dt > 0.0f) {
	...
	// I_min verhindert unrealistisch große Laufzeiten bei sehr kleinem Strom.
...
RobotinoSensorThread::update_akku_monitor(const RobotinoComThread::SensorData &data)
{
	...
	// Spannung nur im Stillstand nach Wartezeit/Mittelung/Korrektur verwenden.
	handle_stillstand_voltage_calibration(data);

	// Während der Fahrt bleibt Coulomb Counting die Hauptrechnung.
